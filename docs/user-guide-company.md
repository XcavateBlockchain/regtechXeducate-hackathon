...
8. [Share modules with customers](#08-share-modules-with-customers)
...
> **Good to know**
> Each account can only have one role: Owner, Employee, or Customer. If you've already used a Google account or wallet to join as an employee or customer, you'll need a different one to register as a company owner.
...
- **Continue with Google (wallet).** The fastest path, ideal for signing into an existing account. This is the option "To access customer or employee account."
- **Continue with wallet (advanced).** Required for creating an organization. This is the option labeled "To access company admin account."
...
>
> The "wallet (advanced)" path routes through Phantom's full sign-in flow, which is required for company registration and admin.
...
2. Each module attempt by an employee or customer consumes one slot.
...
Modules are the core of the platform. Each one is a structured assessment your employees and customers complete to earn a verified credential.
...
## 08 Share modules with customers

Customers are external participants: clients, partners, or third parties who need to complete one or more of your compliance modules but aren't part of your internal team.

Unlike employees, customers don't need an explicit invite. You share a link, they join.
...
![Module detail page with share link area highlighted](screenshots/image16.png)
...
![Customer-facing module join page](screenshots/image17.png)

> **One link per module**
> Each share link is tied to a single module. If you need customers to complete more than one, send them a separate link for each. This keeps each module's enrollment list clean and auditable.

Customers only have access to the modules you share with them. They can't see your dashboard, your team, or anything else about your organization.
...
| Public enrolled | External participants (customers) who have joined via share links. |
| Total enrolments | Combined count of all enrollments across your modules. |

![Company dashboard with stats cards and activity feed](screenshots/image18.png)
...
7.  Share        >  Modules > copy share link > send to customers
